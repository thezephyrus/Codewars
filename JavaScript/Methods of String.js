...
These methods are rarely used in programming, so we will omit the learning of these methods.These methods are:
...
World!
World!
We can see, the first parameter startindex can not be omitted, it represents the beginning of the interception of the string.
 Second parameter can be omitted, the three method will intercept the string to the end of the string. Look at the following example:
...
Hello
Hello
We can see, When the startindex is 0, the results obtained by the three methods are the same. But in fact, their operations are different. 
slice() and substring() intercepted characters from index0 to index5 (not including index5). substr operation is to start from the index0, 
the interception of a substring of length 5. Look at the following example:
...
function cutIt(arr) {
  ...
  for (key in arr) {
    len.push(arr[key].length);
  }

  Array.min = function(arr) {
    return Math.min.apply(Math, arr);
  };
  var smallest = Array.min(len)

  for (i = 0; i < arr.length; i++) {
    x.push(arr[i].substring(0, smallest).trim());
  }
  return x;
}
