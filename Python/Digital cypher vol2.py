...
def decode(word,n):
    ...
    print(digit_list)
    dl2=digit_list*math.ceil((len(word)/len(digit_list)))
    print(dl2)
    base=[]
    result=[]
    for i in range(len(word)):
        base.append(word[i]-dl2[i])
        result.append(reverse_dict[base[i]])
    return (''.join(result))
...
def decode(word, n):
    ...
    digit_list = list(map(int, str(n)))
    dl_len = len(digit_list)
    dl2 = digit_list * -(-len(word) // dl_len)
    result = []
    for i, w in enumerate(word):
        result.append(reverse_dict[w - dl2[i]])
    return ''.join(result)